(function () {
  ...
  // CORREGIDO (XSS): el panel de admin muestra datos de pedidos y clientes
  // que en parte los escribe cualquier visitante de la tienda pública en el
  // checkout (nombre, email, etc). Sin escapar, alguien podría meter un
  // nombre tipo "<img src=x onerror=...>" en el checkout y ese código se
  // ejecutaría en la pantalla del admin al ver la lista de pedidos/clientes.
  ...
  const status = (value) => `<span class="status">${esc(value)}</span>`;
  const nav = (section) =>
    [
      ["dashboard", "▦ Dashboard"],
      ["products", "◈ Productos"],
    ...
    return orders.length
      ? orders
          .map(
            (order) => `
              <tr>
                <td><strong>${esc(order.id)}</strong><br><small class="muted">${date(order.createdAt)}</small></td>
                <td>${esc(order.customer.name)}<br><small class="muted">${esc(order.customer.email)}</small></td>
                <td>${order.items.map((item) => `${Number(item.quantity)}× ${esc(item.name)}`).join("<br>")}</td>
                <td>${esc(order.payment)}<br><small class="muted">${esc(order.deliveryMethod) || "Sin definir"}</small></td>
  ...
  function productForm(id) {
    ...
    return `<section class="container admin-page"><aside class="admin-side"><h3>Administración</h3><nav class="admin-nav">${nav("products")}</nav></aside><div class="admin-content"><div class="admin-bar"><div><p class="eyebrow">${id ? "Editar" : "Nuevo"} producto</p><h1>${id ? esc(p.name) : "Agregar producto"}</h1></div><button class="btn btn-outline btn-sm" data-admin-section="products">← Volver</button></div><form class="checkout-form" id="product-form"><input type="hidden" name="productId" value="${esc(p.id) || ""}"><input type="hidden" name="image" id="product-image-url" value="${esc(p.image) || ""}"><div class="form-grid"><div class="field full"><label>Nombre</label><input name="name" required value="${esc(p.name)}"></div><div class="field full"><label>Descripción</label><textarea name="description" required rows="3">${esc(p.description)}</textarea></div><div class="field"><label>Precio</label><input name="price" type="number" min="1" step="1" required value="${esc(p.price)}"></div><div class="field"><label>Precio anterior (opcional)</label><input name="oldPrice" type="number" min="1" step="1" value="${esc(p.oldPrice) || ""}"></div><div class="field"><label>Stock</label><input name="stock" type="number" min="0" step="1" required value="${esc(p.stock)}"></div><div class="field"><label>Categoría</label><select name="category">${categories.map((c) => `<option ${p.category === c ? "selected" : ""}>${esc(c)}</option>`).join("")}</select></div><div class="field"><label>Marca</label><input name="brand" value="${esc(p.brand) || ""}" placeholder="Marca"></div><div class="field"><label>SKU</label><input name="sku" value="${esc(p.sku) || ""}" placeholder="NOVA-001"></div><div class="field full"><label>Imagen principal</label><div class="image-upload-box"><img id="product-image-preview" src="${esc(p.image) || ""}" alt="Vista previa" style="${p.image ? "" : "display:none;"}max-width:160px;border-radius:10px;margin-bottom:10px;display:block;"><input type="file" id="product-image-file" accept="image/png,image/jpeg,image/webp,image/gif">${id ? '<small class="muted">Dejá este campo vacío para conservar la imagen actual.</small>' : ""}</div></div><div class="field"><label>Etiqueta</label><select name="tag"><option ${p.tag === "Nuevo" ? "selected" : ""}>Nuevo</option><option ${p.tag === "Oferta" ? "selected" : ""}>Oferta</option><option ${p.tag === "Disponible" ? "selected" : ""}>Disponible</option></select></div><div class="field"><label>Estado</label><select name="active"><option value="true" ${p.active !== false ? "selected" : ""}>Activo</option><option value="false" ${p.active === false ? "selected" : ""}>Inactivo</option></select></div><label class="filter-option"><input type="checkbox" name="featured" ${p.featured ? "checked" : ""}>Producto destacado</label><label class="filter-option"><input type="checkbox" name="new" ${p.new ? "checked" : ""}>Marcar como nuevo</label></div><button class="btn btn-primary" id="product-form-submit" style="margin-top:22px">Guardar producto</button></form></div></section>`;
  }
          ...
          .map(
            (customer) => `
              <tr>
                ...
                <td>${esc(customer.email)}<br><small class="muted">${esc(customer.phone) || "Sin teléfono"}</small></td>
                <td>${Number(customer.orders)}</td>
                <td>${money(customer.spent)}</td>
                <td>${customer.lastOrder ? date(customer.lastOrder) : "—"}</td>
                <td><button class="btn btn-outline btn-sm" data-delete-customer="${esc(customer.email)}">Eliminar</button></td>
              </tr>`,
          )
          .join("")
  ...
  function productsSectionView() {
    return `<div class="admin-bar"><div><p class="eyebrow">Catálogo</p><h1>Productos</h1></div><button class="btn btn-primary btn-sm" data-new-product>+ Nuevo producto</button></div><div class="admin-table-wrap"><table class="admin-table"><thead><tr><th>Producto</th><th>SKU</th><th>Precio</th><th>Stock</th><th>Estado</th><th></th></tr></thead><tbody>${products.map((p) => `<tr><td><strong>${esc(p.name)}</strong><br><small class="muted">${esc(p.category)}</small></td><td>${esc(p.sku) || `NOVA-${String(p.id).slice(-3)}`}</td><td>${money(p.price)}</td><td>${Number(p.stock)}</td><td>${status(p.active === false ? "Inactivo" : "Activo")}</td><td><button class="btn btn-outline btn-sm" data-edit-product="${p.id}">Editar</button> <button class="btn btn-sm" data-toggle-product="${p.id}">${p.active === false ? "Activar" : "Desactivar"}</button></td></tr>`).join("")}</tbody></table></div>`;
  }
