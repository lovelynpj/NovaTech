(function () {
  ...
  // CORREGIDO (XSS): todo dato que venga de la base de datos (nombre de
  // producto, descripción, categoría, imagen, whatsapp de settings, nombre
  // de usuario, etc.) pasa por esta función antes de insertarse en HTML.
  // Sin esto, alguien podría cargar un producto con un nombre tipo
  // "<img src=x onerror=alert(1)>" desde el panel de admin y ese código se
  // ejecutaría en el navegador de cualquier visitante de la tienda.
  const esc = window.NovaSecurity.escapeHtml;
  ...
  function productCard(p) {
    ...
    return `<article class="product-card"><span class="tag">${esc(p.tag) || "Disponible"}</span><button class="favorite ${fav ? "active" : ""}" data-fav="${p.id}" aria-label="Favorito">${fav ? "♥" : "♡"}</button><a class="product-image" href="#product/${p.id}"><img src="${esc(p.image)}" alt="${esc(p.name)}" loading="lazy"></a><div class="product-info"><span class="product-category">${esc(p.category)}</span><a href="#product/${p.id}" class="product-title">${esc(p.name)}</a><div class="prices"><span class="price">${money(p.price)}</span>${p.oldPrice ? `<span class="old-price">${money(p.oldPrice)}</span>` : ""}<span class="stock">${stockLeft} disp.</span></div><div class="product-actions"><a class="btn btn-outline btn-sm" href="#product/${p.id}">Ver detalle</a><button class="btn btn-primary cart-add" data-add="${p.id}" aria-label="Agregar al carrito" ${stockLeft <= 0 ? "disabled" : ""}>+</button></div></div></article>`;
  }
  ...
  function home() {
    return `<section class="hero"><div class="hero-content"><p class="eyebrow">Tecnología para tu ritmo</p><h1>Lo mejor de la tecnología, más cerca.</h1><p>Descubrí productos seleccionados que suman diseño, potencia y practicidad a cada día.</p><div class="hero-actions"><a class="btn btn-primary" href="#shop">Comprar ahora →</a><a class="btn btn-outline" href="#shop?category=Audio">Explorar audio</a></div></div></section><section class="container section"><div class="section-heading"><div><p class="eyebrow">Elegidos para vos</p><h2>Productos destacados</h2></div><a href="#shop" class="btn btn-outline">Ver catálogo</a></div><div class="grid products-grid">${products.filter((p) => p.featured).map(productCard).join("")}</div></section><section class="container section"><div class="section-heading"><div><p class="eyebrow">Encontrá lo tuyo</p><h2>Comprá por categoría</h2></div></div><div class="grid categories">${categories.map((c) => {
      ...
      return p
        ? `<a href="#shop?category=${encodeURIComponent(c)}" class="category"><img src="${esc(p.image)}" alt="${esc(c)}"><strong>${esc(c)}</strong><span>Explorar productos →</span></a>`
        : "";
  ...
  function detail(id) {
    ...
    const stockLeft = Number(p.stock) || 0;
    return `<div class="container breadcrumbs"><a href="#home">Inicio</a> / <a href="#shop">Tienda</a> / ${esc(p.category)}</div><section class="container product-detail"><div class="detail-image"><img src="${esc(p.image)}" alt="${esc(p.name)}"></div><div class="detail-info"><p class="eyebrow">${esc(p.category)} · ${esc(p.tag) || "Disponible"}</p><h1>${esc(p.name)}</h1><div class="prices"><span class="price">${money(p.price)}</span>${p.oldPrice ? `<span class="old-price">${money(p.oldPrice)}</span>` : ""}</div><p>${esc(p.description)}</p><p class="stock">● ${stockLeft} unidades disponibles</p><div class="specs">${Object.entries(p.specs || {}).map(([k, v]) => `<div class="spec"><span class="muted">${esc(k)}</span><strong>${esc(v)}</strong></div>`).join("")}</div><div class="buy-actions"><div class="quantity" data-stock="${stockLeft}"><button data-qty="-1">−</button><span id="detail-qty">1</span><button data-qty="1">+</button></div><button class="btn btn-primary" data-detail-add="${p.id}" ${stockLeft <= 0 ? "disabled" : ""}>${stockLeft <= 0 ? "Sin stock" : "Agregar al carrito"}</button></div></div></section><section class="container section"><div class="section-heading"><div><p class="eyebrow">También te puede interesar</p><h2>Productos relacionados</h2></div></div><div class="grid products-grid">${products.filter((x) => x.category === p.category && x.id !== p.id).slice(0, 3).map(productCard).join("")}</div></section>`;
  }
  ...
  function checkout() {
    ...
    const whatsapp = esc(getSettingsCached().whatsapp || "543512379493");
        ...
        <div class="checkout-layout">
          <form class="checkout-form" id="checkout-form">
            ...
            <div class="payment-details" id="transfer-details">
              <strong class="payment-title">Datos para transferir</strong>

              <div class="transfer-card">
                <strong>Mercado Pago</strong>
                <span>Alias</span>
                <div class="alias-row">
                  <code>pedro.pimentel</code>
                  <button type="button" class="copy-alias" data-copy="pedro.pimentel">Copiar</button>
                </div>
              </div>
              ...
              <div class="transfer-card">
                ...
                <span>Alias</span>
                <div class="alias-row">
                  <code>pedro.pimentel.lemon</code>
                  <button type="button" class="copy-alias" data-copy="pedro.pimentel.lemon">Copiar</button>
                </div>
              </div>

              <small>Titular: Pedro Jose Pimentel Quiroz</small>
              <small><strong>Importante:</strong> una vez transferido, envianos el comprobante por <a href="https://wa.me/${whatsapp}?text=Hola%20NovaTech%2C%20te%20paso%20el%20comprobante%20de%20mi%20transferencia." target="_blank" rel="noopener">WhatsApp</a> para confirmar tu pedido.</small>
            </div>

            <div class="payment-details hidden" id="cash-details">
              <strong class="payment-title">Pago en efectivo</strong>
              <p class="muted" style="font-size:13px;margin:0">Coordinamos el pago al momento del retiro o la entrega. Te contactaremos por WhatsApp para confirmar los detalles.</p>
            </div>

            <button class="btn btn-primary" style="margin-top:20px">Confirmar pedido</button>
          </form>
          <aside class="summary"><h3>Tu pedido</h3>${items.map((p) => `<div class="summary-row"><span>${p.quantity} × ${esc(p.name)}</span><span>${money(p.price * p.quantity)}</span></div>`).join("")}<div class="summary-row"><span>Envío</span><span>${shipping ? money(shipping) : "A coordinar"}</span></div><div class="summary-row summary-total"><span>Total estimado</span><span>${money(total)}</span></div></aside>
        </div>
      </section>`;
  }
  ...
  function account() {
    ...
    return `<section class="container account-page"><p class="eyebrow">Mi cuenta</p><h1>Hola, ${esc(user.name)}</h1><div class="account-nav"><button class="active">Mis pedidos</button><button>Favoritos</button><button data-logout>Cerrar sesión</button></div><div class="account-card"><h3>Mis pedidos</h3><div class="empty">Las compras como invitado se consultan desde el email de confirmación.</div></div><p style="margin-top:18px"><a class="btn btn-outline btn-sm" href="#novatech">Acceso administrador</a></p></section>`;
  }
  ...
  function purchasePop() {
    ...
    const show = async () => {
      ...
      // Usa textContent (no innerHTML), así que ya es seguro contra XSS
      // aunque data[0]/data[1] vengan de la base de datos (nombre de
      // cliente, nombre de producto).
  ...
  function whatsappButton() {
    ...
    const phone = encodeURIComponent(getSettingsCached().whatsapp || "543512379493");
  ...
  async function render() {
    ...
    try {
      ...
      app.innerHTML = `<section class="container section"><div class="empty"><h2>No pudimos conectar con el servidor</h2><p class="muted">${esc(err.message)}</p></div></section>`;
      return;
    }
  ...
  document.addEventListener("click", async (e) => {
    ...
    if (add) {
      ...
      // CORREGIDO: antes se podía agregar un producto sin stock (quedaba
      // en el carrito con cantidad 0, un estado roto). Ahora se avisa y
      // no se agrega nada si no hay stock disponible.
      if (!product || !(Number(product.stock) > 0)) {
        toast("Este producto no tiene stock disponible.", true);
      } else {
        addToCart(product);
    ...
    if (e.target.matches("[data-qty]")) {
      ...
      const maxStock = Number(document.querySelector(".quantity")?.dataset.stock ?? Infinity);
      span.textContent = Math.max(1, Math.min(+span.textContent + +e.target.dataset.qty, maxStock || 1));
    }
  ...
  document.addEventListener("change", (e) => {
    ...
    if (e.target.id === "payment-method") {
      ...
      const cash = document.querySelector("#cash-details");
      if (transfer) transfer.classList.toggle("hidden", e.target.value !== "Transferencia");
      if (cash) cash.classList.toggle("hidden", e.target.value !== "Efectivo");
    }
    if (e.target.id === "delivery-method") {
      const pickup = document.querySelector("#pickup-details");
      const shipping = document.querySelector("#shipping-details");
      if (pickup) pickup.classList.toggle("hidden", e.target.value !== "Retiro");
      if (shipping) shipping.classList.toggle("hidden", e.target.value !== "Envío");
    }
  });
  ...
  document.addEventListener("submit", async (e) => {
    ...
    if (e.target.id === "security-form") {
      ...
      const data = Object.fromEntries(new FormData(e.target));
      try {
        // El backend ahora identifica al admin por el token (Authorization
        // header, agregado automáticamente por apiFetch), no por el email
        // que mandemos acá — así que ya no hace falta (ni sirve) mandarlo.
